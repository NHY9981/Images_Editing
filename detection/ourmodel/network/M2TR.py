...
class M2TR(nn.Module):
    ...
    drop_ratio :  the dropout rate
    """
    def __init__(self, img_size, n_dim, drop_ratio, mode = 'train'):
    ...
    def forward(self, x):
        ...
        if self.mode == 'train':
            ...
            return torch.max(out.softmax(dim=-1), dim=1)[1],
...
if __name__ == '__main__':
    ...
    # 创建随机输入数据 (batch_size=1, channels=3, height=224, width=224)
    pic = torch.rand(4, 3, 224, 224).cuda()  # 假设输入尺寸为224x224
    
    # 初始化模型
    model = M2TR(img_size=224, n_dim=768, drop_ratio=0.1)
    
    # 修改最后的全连接层（如果需要）
    # 原模型的fc层输出是2，这里保持原样
    # model.fc = nn.Linear(128 * (img_size//32)**2, 1)  # 如果需要二分类输出
    ...
    # 前向传播
    out = model(pic)
    pred_classes = torch.max(out, dim=1)[1]

    
    # 打印输出
    print("Model output shape:", out.shape)  # 应该输出 [1, 2]
    print("Predicted classes:", pred_classes)  # 应该输出 [1]
